...
class HSBCDocumentProcessor {
  private authToken: string = '';

  // Authenticate and get JWT token
  async authenticate(): Promise<void> {
    try {
      console.log('🔐 Authenticating with HSBC Banking API...');
  ...
  async testKnowledgeBase(): Promise<void> {
    try {
      ...
      const testQueries = [
        ...
        'What are the fees for mortgage loans?',
        'How do I apply for an auto loan?',
        'What digital banking services does HSBC provide?',
        'What are the requirements for a first-time homebuyer?',
...
if (require.main === module) {
  const processor = new HSBCDocumentProcessor();
  processor.processHSBCDocument().catch(console.error);
}

export { HSBCDocumentProcessor };
